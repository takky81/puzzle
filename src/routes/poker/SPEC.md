...
- 公開後はAIの手札の下に**AIの役名**を表示する（自分側と同じ形式）
- 勝者側のパネルには枠のハイライトと `WIN` バッジを付ける
...
- **そのハンドの収支**を両者分、`+`/`-` 付きで表示する（アンティやベットも即時反映）
- ハンド決着後は勝敗の見出し（`あなたの勝ち！` / `AIの勝ち` / `引き分け`）と決着理由を表示する
- **背景色で勝敗を示す**: あなたの勝ち＝赤 / あなたの負け＝青 / 未決着・引き分け＝緑
...
- 自分の手札5枚（表向き）
- 現在の役名を常時表示（例: 「ワンペア（K）」）
- **手札は役が読み取りやすい順に自動で並び替える**（同ランクをまとめて前に置き、同枚数どうしはランク降順。A-2-3-4-5 は 5-4-3-2-A の順）
...
- ハンド終了ごとの結果は盤面上に直接表示する（勝敗＝ポットエリアの見出しと背景色、役＝各プレイヤーの手札の下、増減＝ポットエリアの収支）
...
export function sortedHandIndices(cards: Card[]): number[]; // 役が読みやすい並び順（元のインデックス）
...
- ショーダウンでAIの手札が公開され、AIの役名が手札の下に表示される
- ハンド決着時にポットエリアの見出し・背景色（勝ち＝赤 / 負け＝青）と収支が更新される
