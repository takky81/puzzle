<script lang="ts">
  ...
  import type { GameState, Position } from '$lib/numberlink/types';
  ...
  let size = $derived(Number(page.url.searchParams.get('size') ?? '4'));
  let puzzleId = $derived(page.url.searchParams.get('id') ?? '');

  let loading = $state(true);
  let loadError = $state<string | null>(null);
  let nextPuzzleId = $state<string | null>(null);

  const placeholderStage = { size: 4, numbers: [], solution: [] };
  let game = $state<GameState>(createGame(placeholderStage));
  let showSolution = $state(false);
  let isDragging = $state(false);
  let lastCell: Position | null = $state(null);

  function saveCleared(id: string) {
    try {
  ...
  $effect(() => {
    const s = size;
    const targetId = puzzleId;
    loading = true;
    loadError = null;
    showSolution = false;

    loadPuzzleList(s, base, fetch)
      .then((entries) => {
        ...
        nextPuzzleId = idx < entries.length - 1 ? entries[idx + 1].id : null;
        game = createGame(entries[idx]);
        loading = false;
      })
      .catch((err) => {
  ...
  $effect(() => {
    if (game.isCleared && puzzleId) {
      saveCleared(puzzleId);
    }
  });
  ...
  let gridSize = $derived(stage.size);
...
<div class="numberlink mx-auto max-w-[480px] select-none">
  <h1 class="mb-2 text-2xl font-bold text-primary">ナンバーリンク</h1>
  ...
  {#if loading}
    <div class="board" style="grid-template-columns: repeat(4, 1fr);"></div>
        ...
        {#each indices as c (r * gridSize + c)}
          ...
          >
            {#if numberId !== undefined}
              <span class="number">{numberId}</span>
            {/if}
  ...
  {#if game.isCleared}
    <div class="mt-3 flex flex-wrap justify-center gap-2">
      <a
        href={resolve('/numberlink', {})}
        class="cursor-pointer rounded-md bg-gray-500 px-4 py-2 text-sm font-bold text-white no-underline hover:opacity-80"
      >
        一覧に戻る
      </a>
      {#if nextPuzzleId}
        <a
          href="{resolve('/numberlink/play', {})}?size={size}&id={nextPuzzleId}"
          class="cursor-pointer rounded-md bg-[var(--c-clear)] px-4 py-2 text-sm font-bold text-white no-underline hover:opacity-80"
        >
          次の問題
        </a>
      {/if}
  ...
  {:else}
    <div class="mt-3 flex flex-wrap justify-center gap-2">
      ...
      <a
        href={resolve('/numberlink', {})}
        class="cursor-pointer rounded-md bg-gray-400 px-4 py-2 text-sm font-bold text-white no-underline hover:opacity-80"
      >
        一覧に戻る
      </a>
    </div>
